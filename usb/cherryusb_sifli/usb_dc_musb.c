...
int usbd_ep_clear_stall(uint8_t busid, const uint8_t ep)
{
    ...
    if (USB_EP_DIR_IS_OUT(ep)) {
        ...
        if (ep_idx == 0x00) {
            ...
            // Drop any stale packet before resetting the data toggle.
            HWREGB(USB_TXCSRL_BASE(ep_idx)) =
...
static void handle_ep0(void)
{
    ...
    switch (usb_ep0_state) {
        ...
        case USB_EP0_STATE_OUT_DATA:
            if (ep0_status & USB_CSRL0_RXRDY) {
                read_count = HWREGH(USB_RXCOUNT_BASE(ep_idx));
                read_count = MIN(read_count, g_musb_udc.out_ep[0].xfer_len);
                ...
                if ((read_count < g_musb_udc.out_ep[0].ep_mps) ||
                    (g_musb_udc.out_ep[0].xfer_len == 0)) {
