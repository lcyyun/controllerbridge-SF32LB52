...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="sf32lb52_ds5_feature_") as temp:
        executable = pathlib.Path(temp) / "test_ds5_feature.exe"
        command = [
            compiler,
            "-std=c11",
            "-Wall",
            ...
            "-I",
            str(ROOT / "include"),
            str(ROOT / "protocol" / "sf32lb52_ds5_feature.c"),
            str(ROOT / "tools" / "test_ds5_feature.c"),
            "-o",
            str(executable),
        ]
        env = os.environ.copy()
    ...
    print("OK DS5 feature cache/fallback tests")
    return 0
